 ...
 */
export function nextPrice(policy, counterpartyPrice) {
    ...
    if (policy.role === "buyer") {
        ...
        if (candidate - counterpartyPrice < minDelta)
            candidate = counterpartyPrice + minDelta;
        if (candidate > policy.price)
        ...
        if (counterpartyPrice - candidate < minDelta)
            candidate = counterpartyPrice - minDelta;
        if (candidate < policy.price)
            candidate = policy.price;
        return round2(candidate);
...
export function accepts(policy, counterpartyPrice) {
    if (policy.role === "buyer")
 ...
 */
export function negotiate(wsUrl, policy, identity, opts = {}) {
    const log = opts.log ?? (() => { });
    const minDelta = policy.minDelta ?? DEFAULT_MIN_DELTA;
    ...
    return new Promise((resolve, reject) => {
        ...
        const join = () => {
            ...
            if (policy.role === "buyer")
                joinMsg.identity.maxUnitPrice = policy.price;
            else
                joinMsg.identity.floorUnitPrice = policy.price;
            ws.send(JSON.stringify(joinMsg));
            joined = true;
            log(`${policy.role} joined room`);
        };
        const counteroffer = (price) => {
            send({
                type: "counteroffer",
                payload: {
                    unitPrice: price,
