...
import {
  ...
  submitSellerCompletion,
  submitBuyerDecision,
  submitAdminOverride,
  getVerificationDashboardView,
  getVerificationDashboardViews,
} from "./settle.js";
...
function getOrCreateRoom(roomId: string): Room {
  ...
  if (!room) {
    ...
    if (maxRounds && !Number.isNaN(maxRounds)) config.maxRounds = maxRounds;
    const minDelta = process.env.NEGOTIATE_MIN_DELTA ? Number(process.env.NEGOTIATE_MIN_DELTA) : undefined;
    if (minDelta !== undefined && !Number.isNaN(minDelta)) config.minDelta = minDelta;
    const minMoveFraction = process.env.NEGOTIATE_MIN_MOVE_FRACTION
      ? Number(process.env.NEGOTIATE_MIN_MOVE_FRACTION)
      : undefined;
    if (minMoveFraction !== undefined && !Number.isNaN(minMoveFraction)) config.minMoveFraction = minMoveFraction;
...
export function startServer(port = PORT): WebSocketServer {
  const httpServer = createServer(async (req, res) => {
    ...
    // moment it actually passes (see settle.ts). The frontend hits these via
    ...
    if (req.method === "POST" && actionM) {
      ...
      try {
        ...
        if (action === "seller-complete") {
          record = await submitSellerCompletion(
            requestId,
            String(body.sellerAgentId ?? ""),
            String(body.proof ?? ""),
            body.notes !== undefined ? String(body.notes) : undefined,
          );
        } else if (action === "buyer-decision") {
          record = await submitBuyerDecision(
            requestId,
            String(body.buyerAgentId ?? ""),
            body.decision === "approve" ? "approve" : "reject",
            body.rejectionReason !== undefined ? String(body.rejectionReason) : undefined,
            body.notes !== undefined ? String(body.notes) : undefined,
          );
        } else {
          const decision = body.decision === "rejected" || body.decision === "error" ? body.decision : "verified";
          record = await submitAdminOverride(
            requestId,
            String(body.adminAgentId ?? ""),
            decision,
            body.rejectionReason !== undefined ? String(body.rejectionReason) : undefined,
            body.notes !== undefined ? String(body.notes) : undefined,
          );
        ...
        sendJson(res, 404, { ok: false, error: String((err as Error).message ?? err) });
      }
      return;
    }
  ...
  const baseClose = wss.close.bind(wss) as (cb?: (err?: Error) => void) => void;
