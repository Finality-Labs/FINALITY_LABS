...
import { NegotiationBrain } from "./brain.js";
...
function validate(state) {
    ...
    if (!d)
        throw new Error("no decision produced");
...
export async function runTurn(ctx, input) {
    const graph = buildNegotiationGraph();
    const out = await graph.invoke({ ctx, input, guarded: false });
    return { decision: out.decision, guarded: out.guarded };
}
