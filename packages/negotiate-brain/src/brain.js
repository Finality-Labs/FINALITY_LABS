...
function systemPrompt(ctx) {
    const repNote = ctx.counterparty
        ? `Counterparty (${ctx.counterparty.agentId}) trust score: ${ctx.counterparty.score.toFixed(2)} (${ctx.counterparty.mode}, ${ctx.counterparty.count} ratings). ` +
...
function fallbackDecision(ctx, input) {
    ...
    if (ctx.role === "buyer") {
        if (move - opp < minDelta)
            move = roundPrice(opp + minDelta);
        if (move > ctx.price)
            move = ctx.price;
    }
    else {
        if (opp - move < minDelta)
            move = roundPrice(opp - minDelta);
        if (move < ctx.price)
            move = ctx.price;
    }
...
export class NegotiationBrain {
    ...
    async decide(input) {
        ...
        try {
            ...
            console.warn(`[brain] LLM decision failed (${e.message}); using deterministic fallback`);
            return fallbackDecision(this.ctx, input);
        }
    }
