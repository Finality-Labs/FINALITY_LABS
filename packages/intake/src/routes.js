...
export function registerRoutes(app, store, matchmaker, pulse) {
    app.get("/health", async () => ({ ok: true }));
    app.post("/intents", async (req, reply) => {
        const parsed = intentSchema.safeParse(req.body);
        if (!parsed.success)
            return reply.code(400).send({ error: parsed.error.message });
        const id = store.addIntent(parsed.data);
    ...
    app.post("/offers", async (req, reply) => {
        ...
        if (!parsed.success)
            return reply.code(400).send({ error: parsed.error.message });
        const id = store.addOffer(parsed.data);
        pulse.register(id, parsed.data.pulseMinutes);
    ...
    app.get("/matches/:id", async (req, reply) => {
        ...
        if (!m.matched)
            return reply.code(200).send({ matched: false });
        return reply.code(200).send(m);
    });
    app.get("/agents/:agentId/reputation", async (req) => {
    ...
    app.post("/_identity", async (req) => {
        ...
        return verifyOrRegister(parsed.data);
    });
    ...
    app.post("/offers/:id/pulse", async (req, reply) => {
    ...
    app.get("/offers/:id/registry", async (req, reply) => {
    ...
    // call this.
    app.post("/registry/:agentId/notify", async (req, reply) => {
    ...
    app.get("/registry/:agentId", async (req) => {
